// T-Rex Runner — Custom Edition
// Original engine: © The Chromium Authors (chromium.googlesource.com/chromium/src/)
// Extracted from Chromium source by @liuwayong
// Custom modifications, audio & skins © 2026 Noeal Rajeev T. — All rights reserved.
(function () {
  "use strict";

  // ── bichootyy custom audio ──────────────────────────────────────────────────
  var BICHOOTYY_MODE =
  ...
  if (BICHOOTYY_MODE) {
    // Audio elements can load local files without CORS restrictions.
    // Setting preload='auto' fetches the file as soon as the element is created.
    _biJumpEl = new Audio("assets/audio/play.ogg");
  ...
  function _biPlayJump() {
    if (!_biJumpEl) return;
    // Cancel any pending pause left over from a previous jump.
    // Without this, stacked timeouts from rapid jumps fire mid-playback
    // and cut the audio short.
    if (_biJumpTimer !== null) {
      clearTimeout(_biJumpTimer);
      _biJumpTimer = null;
    }
    _biJumpEl.pause();
    _biJumpEl.currentTime = 0;
    var p = _biJumpEl.play();
    _biJumpTimer = setTimeout(function () {
      _biJumpEl.pause();
      _biJumpTimer = null;
    }, 1350);
  ...
  /** Play game-over sound: out.ogg in full. */
  function _biPlayOut() {
    if (!_biOutEl) return;
    _biOutEl.pause();
